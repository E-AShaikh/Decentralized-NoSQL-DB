...
import com.db.config.tcp.UserConnection;
import com.db.config.udp.UdpManager;
import com.db.model.Node.Node;
import com.db.network.DockerNetwork;
...
public class Bootstrapper {
    ...
    private void createNetwork() throws IOException, ExecutionException, InterruptedException, TimeoutException, ParseException {
        ...
        for(int i = 1; i<= Config.getInstance().getContainerNumbers(); i++,tcpPort++,udpPort++,bootstrapperPort++){
            try {
                Node node = new Node();
    ...
    private class TcpListener implements Runnable{
        @Override
        public void run() {
            try (ServerSocket serverSocket = new ServerSocket(Config.getInstance().getBootstrapperTcpPort())){
                while (true) {
